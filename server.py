...
def sort_month_columns(ws) -> None:
    headers = [clean_text(ws.cell(1, col_index).value) for col_index in range(1, ws.max_column + 1)]
    base_headers = [header for header in BASE_HEADERS if header in headers]
    extra_headers = [
        header
        for header in headers
        if header and header not in base_headers and not re.match(r"\d{4}년 \d{1,2}월", header)
    ]
    month_headers_sorted = sorted(
        [header for header in headers if re.match(r"\d{4}년 \d{1,2}월", header)],
        key=month_sort_key,
    )
    ordered_headers = base_headers + extra_headers + month_headers_sorted
    ...
    for row_index in range(1, ws.max_row + 1):
        old_values.append(
            {headers[col_index - 1]: ws.cell(row_index, col_index).value for col_index in range(1, ws.max_column + 1)}
        )

    for row_index, row_values in enumerate(old_values, start=1):
        for col_index, header in enumerate(ordered_headers, start=1):
            ws.cell(row_index, col_index).value = header if row_index == 1 else row_values.get(header)
...
def search_master(query: str, limit: int = 20) -> list[dict[str, Any]]:
    ...
    for row_index in range(2, ws.max_row + 1):
        has_usage = any(
            parse_number(ws.cell(row_index, headers.index(month) + 1).value) != 0 for month in months
        )
        if not has_usage:
            continue
...
class AppHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if path == "/":
            root_index = RESOURCE_ROOT / "index.html"
            self.serve_file(root_index if root_index.exists() else STATIC_DIR / "index.html", "text/html; charset=utf-8")
        elif path == "/app.js":
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            # multipart/form-data 파싱 (email 모듈 사용)
            ...
            for part in msg.get_payload():
                ...
                entry = type("Field", (), {"filename": filename, "file": io.BytesIO(payload), "value": payload.decode(errors="replace")})()
                fields.setdefault(name, []).append(entry)

            month_entries = fields.get("month", [])
            month = month_entries[0].value if month_entries else ""
            source_items = fields.get("source", [])
            pivot_items = fields.get("pivot", [])
            pivot_item = pivot_items[0] if pivot_items else None
