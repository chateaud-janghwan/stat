...
function buildWeightedDeltaForecast(series) {
  const recent = series
  ...
  return {
    ...
    label: "최근 변화 가중",
    method: "weighted-delta",
  };
}

function holtDampedForecast(values, alpha, beta, phi) {
  let level = values[0];
  let trend = values[1] - values[0];
  let error = 0;

  for (let index = 1; index < values.length; index += 1) {
    const predicted = Math.max(0, level + phi * trend);
    ...
    error += ((actual - predicted) / scale) ** 2;
  }

  return {
    value: Math.max(0, level + phi * trend),
    error,
  };
}

function buildMovingAverageForecast(series) {
  const recent = series
    .map((point, index) => ({ index, value: Number(point.value || 0) }))
    .filter((point) => point.value > 0)
    .slice(-3);
  ...
  return {
    value: recent.reduce((sum, point) => sum + point.value, 0) / recent.length,
    latestIndex: recent.at(-1).index,
    nextIndex: series.length,
    label: "최근 3개월 평균",
...
function buildLinearRegressionForecast(series) {
  const points = series
    .map((point, index) => ({ index, value: Number(point.value || 0) }))
    .filter((point) => point.value > 0)
    .slice(-6);
  ...
  return {
    value: Math.max(0, intercept + slope * series.length),
    latestIndex: points.at(-1).index,
    nextIndex: series.length,
    label: "최근 6개월 선형",
...
function buildHoltForecast(series) {
  const positive = series
    .map((point, index) => ({ index, value: Number(point.value || 0) }))
    .filter((point) => point.value > 0);

  if (positive.length < 6) {
    return buildWeightedDeltaForecast(series);
  }

  const values = positive.map((point) => point.value);
  let best = null;

  for (const alpha of [0.2, 0.35, 0.5, 0.65, 0.8]) {
    for (const beta of [0.05, 0.15, 0.3, 0.45]) {
      for (const phi of [0.8, 0.9, 0.98]) {
        const candidate = holtDampedForecast(values, alpha, beta, phi);
        if (!best || candidate.error < best.error) {
          best = { ...candidate, alpha, beta, phi };
        }
      }
    }
  }
...
function buildForecasts(series) {
  const forecasts = [
    buildHoltForecast(series),
    buildWeightedDeltaForecast(series),
    buildMovingAverageForecast(series),
    buildLinearRegressionForecast(series),
  ].filter(Boolean);
  const colors = ["#7c3f18", "#2166ac", "#3c8c54", "#8a4fa3"];
  return forecasts.map((forecast, index) => ({ ...forecast, color: colors[index] }));
}
...
function renderChart(item, series, components = []) {
  ...
  const forecasts = buildForecasts(series);
  const forecastMonth = nextMonthLabel(series.at(-1)?.month || "");
  const chartSeries = forecasts.length
    ? [...series, { month: forecastMonth, value: Math.max(...forecasts.map((forecast) => forecast.value)), changePct: null, forecast: true }]
    : series;
  const width = Math.max(920, 76 + 36 + (chartSeries.length - 1) * 72);
  const height = 450;
  const margin = { top: 34, right: 36, bottom: 106, left: 76 };
  const monthLabelY = height - margin.bottom + 36;
    ...
    components.length > 1
      ? components
          .map((component, componentIndex) => {
            ...
            return `
              <polyline class="component-line animated-line" style="stroke:${color}" points="${componentPoints}" pathLength="1"></polyline>
            `;
          })
          .join("")
      : "";
  const componentLegend =
    components.length > 1
      ...
      : "";
  const forecastLine = forecasts.length
    ? forecasts
        .map((forecast) => {
          const y = yFor(forecast.value);
          return `
            <line class="forecast-line animated-line forecast-animated" style="stroke:${forecast.color}" x1="${xFor(forecast.latestIndex)}" y1="${yFor(series[forecast.latestIndex].value)}" x2="${xFor(forecast.nextIndex)}" y2="${y}" pathLength="1"></line>
            <circle class="forecast-dot" style="stroke:${forecast.color}" cx="${xFor(forecast.nextIndex)}" cy="${y}" r="6"></circle>
          `;
        })
        .join("")
    : `<text class="model-label" x="${width - margin.right}" y="${margin.top - 10}" text-anchor="end">예측 계산 데이터 부족</text>`;
  const forecastSummary = forecasts.length
    ? `
          ...
          .map(
            (forecast) => `
              <div class="forecast-card">
                <span class="forecast-swatch" style="background:${forecast.color}"></span>
                <span class="forecast-card-label">${forecast.label}</span>
                <strong>${formatNumber(forecast.value)}</strong>
              </div>
            `,
          )
          .join("")}
  ...
  const dots = chartSeries
    .map((point, index) => {
      ...
      if (point.forecast) {
        return `
          <g>
            <text class="month-label forecast-month" x="${x}" y="${monthLabelY}" text-anchor="end" transform="rotate(-38 ${x} ${monthLabelY})">${point.month.replace("년 ", ".").replace("월", "")}</text>
          </g>
        `;
      }
      ...
      return `
        <g>
          ...
          <text class="month-label" x="${x}" y="${monthLabelY}" text-anchor="end" transform="rotate(-38 ${x} ${monthLabelY})">${point.month.replace("년 ", ".").replace("월", "")}</text>
        </g>
      `;
    })
    .join("");
  ...
  chartMount.innerHTML = `
    <div class="chart-scroll">
      <svg class="trend-chart" style="width:${width}px" viewBox="0 0 ${width} ${height}" role="img" aria-label="월별 사용량 그래프">
        ${grid}
        ...
        <text class="axis-title" x="${width / 2}" y="${height - 8}" text-anchor="middle">연월</text>
        <text class="axis-title" x="18" y="${height / 2}" text-anchor="middle" transform="rotate(-90 18 ${height / 2})">사용량</text>
        ${componentLines}
        <polyline class="trend-line animated-line" points="${points}" pathLength="1"></polyline>
        ${forecastLine}
        ${componentLegend}
        ${dots}
      </svg>
